"""apl preview -- show local-only fields, provider payloads, estimated exposure."""
...
def run(example_dir: str) -> int:
    ...
    for provider, payload_path in paths["payloads"].items():
        ...
        print(f"\n-- {provider.upper()} SEES ({info['exposure_ratio']:.1%} "
              f"of original, {info['chars']} chars) " + "-" * 8)
        print(c.read_text(payload_path).strip())
    ...
    print(f"  max_single_provider_exposure: "
          f"{view['max_single_provider_exposure']:.1%}")
    print(f"  no_single_provider_saw_full: {view['no_single_provider_saw_full']}")
    print("\nNote: P0 uses user-guided masking and curated provider payloads.")
    return 0
