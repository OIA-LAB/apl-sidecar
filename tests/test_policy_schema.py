...
def test_example_receipts_match_schema():
    for ex in ("00_private_matter", "01_private_code_context"):
...
def test_schema_rejects_bad_receipt():
    ...
    r2 = _load(REPO / "examples" / "00_private_matter" / "receipt.json")
    r2["max_single_provider_exposure"] = 1.5
    assert check(r2, RECEIPT_SCHEMA) == []  # >1 is valid by design
    r3 = _load(REPO / "examples" / "00_private_matter" / "receipt.json")
